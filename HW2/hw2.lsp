...
; Question 1 ;
;;;;;;;;;;;;;;
(defun BFS(FRINGE) ;create the recursive function
    (cond ((not FRINGE) nil) ;if we have a null element, just return
        (
            (not (listp (first FRINGE))) ; else if the first element of the list is a list
            (cons (first FRINGE) (BFS (cdr FRINGE)))
            ; we reorder the current list, take the first element and add it to the back of the list and call BFS on the new list
            ;this reordering lets us visit the elements on the level that are leaf nodes, and then lets us come back and explore the elements that act as parent nodes
        )
        (t (BFS (append (cdr FRINGE) (first FRINGE)))) ; if the current element at the level is a atomic item, we remove it and create a list of it and the output of calling bfs on the rest of
        ; the unexplored tree
        ; append dereferences the outer list so we will explore those elements
    )
)
