...
;
;perform basic algebra to get our value
...
;
; pair the current color with every other one
...
; 
;we select one color to be colored, then we make a bunch of 2 item clauses that show that only the desired color can be colored
; call the helper function because for every non desired color we create a pair of negated values showing they cant be selected
...
;for each respective node pair, we show which colors cannot be colored at the same time
